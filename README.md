...
Blinks/pupil oculisons are recorded as NA, 0 or even negative time observations in the raw signal. Partial occlusions of the pupil are not easily detectable and one should examine the data after the removal of these points to check for other unusual activity. All these activity is usually removed from 100 ms before until 200 ms after the rapid closing of the eyelid. To solve the issue of the missing observations, we implemented in the R function `pup.med` three different kinds of data imputation: Gaussian [3], t-Student [2] and Kalman filtering [3]. 
